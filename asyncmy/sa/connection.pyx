...
from asyncmy.sa.engine import Engine
from asyncmy.sa.result import create_result_proxy
from asyncmy.sa.transaction import (RootTransaction, Transaction,
                                    NestedTransaction, TwoPhaseTransaction)
from asyncmy.sa.contexts import _TransactionContextManager, _SAConnectionContextManager
from asyncmy.connection import Connection
from asyncmy.sa import exc

cdef inline noop(k):
    return k


cdef class SAConnection:
    cdef:
        object _connection
        object _transaction
        int _savepoint_seq
        object _weak_results
        object _engine
        object dialect
        dict compiled_cache

    def __init__(self, connection: Connection, engine: Engine,
                 compiled_cache: Optional[Dict[ClauseElement, Compiled]] = None):
    ...
    async def _execute(self, query, *multiparams, **params):
        async with self._connection.cursor() as cursor:
            ...
            if isinstance(query, str):
                ...
                compiled, post_processed_params, result_map = _prepare_clause(self, dp, query, result_map)
            ...
            ret = await create_result_proxy(
                self, cursor, self.dialect, result_map
            )
            self._weak_results.add(ret)
            return ret
    ...
    async def _savepoint_impl(self, name=None):
        ...
        name = 'asyncmy_sa_savepoint_%s' % self._savepoint_seq
...
cdef _base_params(conn: SAConnection, query, dp, compiled, is_update):
    """
    handle params
    """
    ...
    post_processed_params = conn.dialect.execute_sequence_format(params)
    return post_processed_params[0]

cdef _prepare_clause(conn: SAConnection, dp, query: ClauseElement, result_map):
    if conn.compiled_cache is not None:
        key = query
        compiled = conn.compiled_cache.get(key)
        if not compiled:
            ...
            if dp and dp.keys() == compiled.params.keys() \
                ...
                conn.compiled_cache[key] = compiled
    else:
        compiled = query.compile(dialect=conn.dialect)
    if not isinstance(query, DDLElement):
        post_processed_params = _base_params(
            conn,
            query,
            dp,
            compiled,
        ...
        result_map = None
    return compiled, post_processed_params, result_map

cdef _distill_params(multiparams: Optional[List[BindParameter]], params: Optional[Dict[str, BindParameter]]):
